...
public class GuestEnglishPDF {
    public static void main(String[] args) {
        ...
        try (FileInputStream fis = new FileInputStream(excelFilePath);
            ...
            for (int i = 1; i < rowCount; i++) { // Skip the first row (header)
                ...
                if (row != null) {
                    Cell cell1 = row.getCell(4, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
           ...
           // generate pdf single
            ...
            String outputPdf = "C:\\reactjs\\ics\\pdf\\Guest_SNVN.pdf";  // Final merged PDF
    ...
    private static void createPDF(List<String> names, int fileIndex) {
        String inputPdf = "C:\\reactjs\\ics\\reserve\\GUEST BOX.pdf";
        ...
        try {
            ...
            for (int i = 0; i < names.size(); i++) {
                ...
                Font font = new Font(calibriBold, fontSize, Font.BOLD, color);//12
           if(i==0)
           {
               ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), 165, 625 , 0);

           }
           else if(i==1){
             //  ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), startX, startY - (i * spacing), 0);
               ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), 418, 623 , 0);

           }

           else if(i==2){
              // ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), startX, startY - (i * spacing), 0);
               ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), 165, 270 , 0);

           }
           else{
              // ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), startX, startY - (i * spacing), 0);
               ColumnText.showTextAligned(canvas, Element.ALIGN_CENTER, new Phrase(names.get(i), font), 418, 273 , 0);
